...
if __name__ == "__main__":
    ...
    sample_data = b'{"m":0,"p":[[48, [0, 0, 84, 0]], [48, [0, 0, -54, 0]], [49, [0, 0, 116, 0]], [63, [0, 0, 380]], [61, [0, null, 2, 4, 3]], [62, [null]], [11, 741, 667], [0, 0, 0], [-98, 47, -1], "", 0]}\r'
    
    # Create and parse status
    status = SpikeStatus(sample_data)
    ...
    print(f"Distance sensor reading: {status.sensors.distance}")
    print(f"Motor A position: {status.motors['A'].position}")
    print(f"Motor B position: {status.motors['B'].position}")
    
    if status.sensors.gyro:
        print(f"Gyro X axis: {status.sensors.gyro.x}")
    
    if status.sensors.position:
        print(f"Current position: ({status.sensors.position.x}, {status.sensors.position.y})")