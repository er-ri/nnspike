...
class LegoSpike(object):
    ...
    def send_sensor_data(self):
        ...
        try:
            ...
            print("[DEBUG][send_sensor_data] " + json_string)  # 送信直前のJSONをprint
            bytes_written = self.usb.write(json_string.encode('utf-8') + b'\r\n')
            if bytes_written is None or bytes_written == 0:
                print("[ERROR][send_sensor_data] USB write failed")
        except Exception as e:
            print("[ERROR][send_sensor_data] Exception:", e)
...
def main_loop():
    ...
    sensor_send_counter = 0  # 4回に1回だけセンサーデータ送信
...
print("[DEBUG] Hub initialized")

try:
    lego_spike = LegoSpike()
    lego_spike.send_sensor_data()  # 起動直後に必ず1回デバッグ出力
    main_loop()
finally:
    try:
