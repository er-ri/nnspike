...
flowchart LR
    ...
    G2[スマートキャリー2]
    Goal((ゴール))
    Start --> S1 --> OB --> S2 --> C1 --> S3 --> C2 --> DL1 --> DL2 --> DL3 --> G1 --> G2 --> Goal
    DL2 --ループ--> DL1
```

### コース区間表（区間ごとの特徴・推奨パラメータ一覧）

| 区間名                | 開始位置      | 終了位置      | 推奨power値 | 主な動作・注意点                  | 備考                |
|----------------------|--------------|--------------|------------|-----------------------------------|---------------------|
| 直線区間1            | スタート      | 障害物手前    | 70～80     | 加速・最高速重視                  | STRAIGHT_POWER      |
| 障害物区間           | 直線1終端    | 直線2始端    | 40～55     | 障害物回避・減速                  | BASE_POWER          |
| 直線区間2            | 障害物終端    | カーブ1始端  | 70～80     | 再加速                            | STRAIGHT_POWER      |
| 90度カーブ1          | 直線2終端    | 直線3始端    | 35～45     | 脱線防止・減速                    | BASE_POWER          |
| 直線区間3            | カーブ1終端  | カーブ2始端  | 70～80     | 加速                              | STRAIGHT_POWER      |
| 90度カーブ2          | 直線3終端    | ダブルループ | 35～45     | 脱線防止・減速                    | BASE_POWER          |
| ダブルループ進入     | カーブ2終端  | ループ内     | 30～40     | 安定走行・最徐行                  | CURVE_POWER         |
| ダブルループ内カーブ | ループ進入   | ループ出口   | 30～40     | 安全最優先                        | CURVE_POWER         |
| ダブルループ出口     | ループ内終端 | キャリー1    | 40～55     | 直線復帰・加速                    | STRAIGHT_POWER      |
| スマートキャリー1    | ループ出口   | キャリー2    | 30～45     | 減速・安定走行・ボトル運搬        | BASE_POWER          |
| スマートキャリー2    | キャリー1終端| ゴール       | 30～45     | 減速・安定走行・ゲート通過        | BASE_POWER          |
