...
from nnspike.models import NvidiaModelWithMode
...
def main(model_path, record_sensor_data=False, save_camera_video=False, send_video_stream=False, course="left"):
    ...
    # Initialize model
    # model = NvidiaModelWithMode(num_modes=6)
    ...
    vs = WebcamVideoStream(src=0, save_video=save_camera_video, timestamp=TIMESTAMP)
    vs.start()
    ...
    try:
        while et.is_running == True:
            ...
            mode_output = outputs[0][0]  # Remove batch dimension

            predicted_x = x1 + (outputs[1][0] * (x2 - x1))
