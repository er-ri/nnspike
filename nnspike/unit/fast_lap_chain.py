...
class FastLapChain(object):
    ...
    #内側走り1
    def fast_lap5(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        if phase.get_phase() == 1:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=80.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 80.0)
                else:
                    self.et.set_start_yaw(start_yaw + 80.0)
            else:
                if self.course == "right":
                    ...
                    return None, (100, 80, 0), Mode.FAST_LAP
        ...
        # フェーズ3: 左旋回60度判定（ここでは20度基準）。到達で次フェーズ、基準yaw更新（左旋回区間）
        if phase.get_phase() == 3:
            stop_turn = et.is_yaw_turn_finished(side=self.course, threshold_deg=85.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 85.0)
                else:
                    self.et.set_start_yaw(start_yaw + 85.0)
            else:
                if self.course == "right":
    ...
    def fast_lap2(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        # フェーズ1: 右旋回30度判定（ここでは20度基準）。到達で次フェーズ、基準yaw更新（右旋回区間）
        if phase.get_phase() == 1:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=65.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 65.0)
                else:
                    self.et.set_start_yaw(start_yaw + 65.0)
            else:
                if self.course == "right":
                    return None, (85, 100, 0), Mode.FAST_LAP
                else:
                    return None, (100, 85, 0), Mode.FAST_LAP
        ...
        # フェーズ3: 左旋回60度判定（ここでは20度基準）。到達で次フェーズ、基準yaw更新（左旋回区間）
        if phase.get_phase() == 3:
            stop_turn = et.is_yaw_turn_finished(side=self.course, threshold_deg=60.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 60.0)
                else:
                    self.et.set_start_yaw(start_yaw + 60.0)
            else:
                if self.course == "right":
    ...
    def fast_lap(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        if phase.get_phase() == 1:
            stop_turn = et.is_yaw_turn_finished(side=self.course, threshold_deg=65.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 65.0)  # 右コースは+30度
                else:
                    self.et.set_start_yaw(start_yaw + 65.0)  # 左コースは-30度
            else:
                if self.course == "right":
                    return None, (85, 100, 0), Mode.FAST_LAP
                else:
        ...
        if phase.get_phase() == 3:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=55.0)
            if stop_turn:
                sys.exit("")
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
    ...
    def FAST_LAP2(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        if phase.get_phase() == 0:
            ...
            if position_diff < 1000:
                left_speed, right_speed = et.yaw_straight_control(base_speed=HIGH_SPEED_BASE)
                return None, (left_speed, right_speed, 0), Mode.FAST_LAP2
            else:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
        ...
        if phase.get_phase() == 1:
            ...
            if stop_turn:
                ...
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 20.0)  # 右コースは-20度
                else:
                    self.et.set_start_yaw(start_yaw + 20.0)  # 左コースは+20度
            else:
                if self.course == "right":
                    return None, (90, 100, 0), Mode.FAST_LAP2
                else:
                    return None, (100, 90, 0), Mode.FAST_LAP2
        ...
        # フェーズ3: 左旋回60度（is_yaw_turn_finished判定）。到達で次フェーズ、基準yaw更新（左旋回区間）
        if phase.get_phase() == 3:
            stop_turn = et.is_yaw_turn_finished(side=self.course, threshold_deg=20.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
        ...
        if phase.get_phase() == 5:
            ...
            if stop_turn:
                ...
                if self.course == "right":
                    return None, (70, 100, 0), Mode.FAST_LAP2
                else:
                    return None, (100, 70, 0), Mode.FAST_LAP2
        ...
        print("[FAST_LAP2] Unexpected state reached.")
        return None, None, Mode.FAST_LAP2