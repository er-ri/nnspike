...
class FastLapChain(object):
    ...
    def fast_lap(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        if phase.get_phase() == 1:
            stop_turn = et.is_yaw_turn_finished(side=self.course, threshold_deg=55.0)
            if stop_turn:
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course))
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 55.0)  # 右コースは+30度
                else:
                    self.et.set_start_yaw(start_yaw + 55.0)  # 左コースは-30度
            else:
                if self.course == "right":
                    return None, (80, 100, 0), Mode.FAST_LAP
                else:
