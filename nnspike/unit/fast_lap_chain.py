    ...
    def fast_lap(self, image: np.ndarray) -> Tuple[Tuple[int, int], Mode]:
        ...
        if phase.get_phase() == 0:
            ...
            if position_diff < 5100:
                ...
                left_speed, right_speed = self.et.yaw_straight_control(base_speed=HIGH_SPEED_BASE, deadband = 3.0, adjust_speed = 1) #
        ...
        if phase.get_phase() == 1:
            stop_turn = self.et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=60.0)#
            if stop_turn:
