...
class FastLapChain(object):
    ...
    def fast_lap(self, image: np.ndarray) -> Tuple[Tuple[int, int], Mode]:
        ...
        if phase.get_phase() == 1:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=13.0)
            if stop_turn:
                if self.course == "right":
                    et.set_start_yaw(start_yaw - 13.0)
                else:
                    et.set_start_yaw(start_yaw + 13.0)
                lap_elapsed = time.time() - self.lap_start_time
        ...
        if phase.get_phase() == 3:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=77.0)
            if stop_turn:
                if self.course == "right":
    ...
    def fast_lap2(self, image: np.ndarray) -> Tuple[Tuple[int, int], Mode]:
        if not self._init:
            self.initialize_action(motor_side=self.course)
            self.et.set_start_yaw()
            self.start_yaw = self.et.get_start_yaw()
        ...
        # フェーズ2: 左旋回20度（is_yaw_turn_finished判定）。到達で次フェーズ、基準yaw更新
        if phase.get_phase() == 2:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=17.0)
            if stop_turn:
                if self.course == "right":
                    et.set_start_yaw(start_yaw - 17.0)
                else:
                    et.set_start_yaw(start_yaw + 17.0)
                lap_elapsed = time.time() - self.lap_start_time
        ...
        # フェーズ4: 左旋回90度（is_yaw_turn_finished判定）。到達で次フェーズ、基準yawをstart_yaw-90.0に更新
        if phase.get_phase() == 4:
        ...
        # フェーズ6: 左旋回90度（is_yaw_turn_finished判定）。到達で次フェーズ、基準yawをstart_yaw-180.0に更新
        if phase.get_phase() == 6:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=90.0)
            if stop_turn:
                phase.next_phase(current_pos)
                lap_elapsed = time.time() - self.lap_start_time
                print(f"[DEBUG] mode={Mode.FAST_LAP2.value} | phase={phase.get_phase()} | yaw={self.et.get_yaw():.2f} | current_pos={current_pos} | time: {lap_elapsed:.3f}秒")
                if self.course == "right":
                    self.et.set_start_yaw(start_yaw - 180.0)
                else:
                    self.et.set_start_yaw(start_yaw + 180.0)
                ...
                if self.course == "right":
                    return (72, 100), Mode.FAST_LAP2
                else:
                    return (100, 72), Mode.FAST_LAP2

        # フェーズ7: position_startとの差分200未満ならyaw_straight_control直進。200以上で次フェーズ
        if phase.get_phase() == 7:
            position_diff = phase.get_position_diff(current_pos)
            if position_diff < 200:
                left_speed, right_speed = et.yaw_straight_control(base_speed=HIGH_SPEED_BASE)
                ...
                self.lap_end_time = time.time()
                lap_elapsed = self.lap_end_time - self.lap_start_time
                print(f"[DEBUG] mode={Mode.FAST_LAP2.value} | phase={phase.get_phase()} | position_diff={position_diff} >= 200 | current_pos={current_pos} | time: {lap_elapsed:.3f}秒")
                phase.next_phase(current_pos)
                return (0, 0), Mode.FAST_LAP2

        # フェーズ8: reset_action()してPAUSE復帰（ラップ終了）
