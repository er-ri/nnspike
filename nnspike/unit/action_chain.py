...
class ActionChain(object):
    ...
    def carry_bottle2(self, image: np.ndarray) -> Tuple[Optional[float], Optional[Tuple[int, int]], Mode]:
        ...
        1. FOLLOW_RIGHT_EDGE + find_bottle_center(blue)
           ...
           - blue_detected_timeから3.7秒後に次段階へ遷移
        2. TURN_RIGHT（blue_detected_time+3.7〜5.7秒, 2.0秒右旋回）
        3. FORWARD（blue_detected_time+5.7〜8.7秒, 3.0秒直進）
        4. TURN_LEFT（blue_detected_time+8.7〜9.5秒, 0.8秒左旋回）
        5. GATE_PASS（blue_detected_time+9.5〜13.5秒, 4.0秒get_virtual_line_edges_at_yで直進）
        6. TURN_LEFT（blue_detected_time+13.5〜14.3秒, 0.8秒左旋回）
        7. EYE_BLUE（blue_detected_time+14.3秒以降, 2.2秒で停止）
        """
        ...
        # blue_pixel_countが一度3000以上になってから3.7秒経過で次段階へ
        if state["blue_detected_time"] is None or (self.current_time - state["blue_detected_time"] < 3.7):
        ...
        # 2. TURN_RIGHT (blue_detected_time+3.7〜5.7秒, 2.0秒右旋回)
        elif self.current_time - state["blue_detected_time"] < 5.7:
            left_speed, right_speed = 60, 0  # 右旋回
            return None, (left_speed, right_speed), Mode.CARRY_BOTTLE2

        # 3. FORWARD (blue_detected_time+5.7〜8.7秒, 3.0秒直進)
        elif self.current_time - state["blue_detected_time"] < 8.7:
            left_speed = right_speed = BASE_SPEED
            return None, (left_speed, right_speed), Mode.CARRY_BOTTLE2
        ...
        # 5. GATE_PASS (blue_detected_time+9.5〜13.5秒, 4.0秒get_virtual_line_edges_at_yで直進)
        elif self.current_time - state["blue_detected_time"] < 13.5:
            pre_target_x = state.get("pre_target_x")
