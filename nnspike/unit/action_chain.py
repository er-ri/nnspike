    ...
    def high_speed_avoid(self, image: np.ndarray) -> Tuple[Optional[float], Optional[SpeedTuple], Mode]:
        ...
        if phase.get_phase() == 7:
            ...
            if corner_detected and position_diff >= 400:
                print(f"[DEBUG] phase7→phase8: position_diff={position_diff} current_pos={current_pos} >= 400 and corner_detected")
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course, status=status))
            else:
                target_x = self.get_target_x_by_course_safe(image, self.course)
        ...
        if phase.get_phase() == 8:
            ...
            # 一定距離進んだら、垂直黒ライン判定
                ...
                print(f"[DEBUG] phase8→phase9: position_diff={position_diff} current_pos={current_pos} >= 700 and vertical_black_line_detected")
        ...
        if phase.get_phase() == 10:
            ...
            if position_diff > 1300:
                ...
                print(f"[DEBUG] phase10→phase11: position_diff={position_diff} current_pos={current_pos} >= 700 and vertical_black_line_detected")
                phase.next_phase()
                phase.set_position_start("position_start", self.get_motor_position(self.course, status=status))
            else:
                if self.course == "right":
