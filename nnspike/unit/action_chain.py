...
class ActionChain(object):
    ...
    def carry_bottle2(self, image: np.ndarray) -> Tuple[Optional[float], Optional[Tuple[int, int]], Mode]:
        ...
        2. TURN_RIGHT（blue_detected_time+3.5〜5.0秒, 1.5秒右旋回）
        3. FORWARD（blue_detected_time+5.0〜8.0秒, 3.0秒直進）
        4. TURN_LEFT（blue_detected_time+8.0〜8.8秒, 0.8秒左旋回）
        5. GATE_PASS（blue_detected_time+8.8〜12.8秒, 4.0秒get_virtual_line_edges_at_yで直進）
        6. TURN_LEFT（blue_detected_time+12.8〜13.6秒, 0.8秒左旋回）
        7. EYE_BLUE（blue_detected_time+13.6秒以降, 2.2秒で停止）
        """
        ...
        # 2. TURN_RIGHT (blue_detected_time+3.5〜5.0秒, 1.5秒右旋回)
        elif self.current_time - state["blue_detected_time"] < 5.0:
        ...
        # 4. TURN_LEFT (blue_detected_time+8.0〜8.8秒, 0.8秒左旋回)
        elif self.current_time - state["blue_detected_time"] < 8.8:
        ...
        # 6. TURN_LEFT (blue_detected_time+12.8〜13.6秒, 0.8秒左旋回)
        elif self.current_time - state["blue_detected_time"] < 13.6:
            left_speed, right_speed = 0, 60
            return None, (left_speed, right_speed), Mode.CARRY_BOTTLE2
