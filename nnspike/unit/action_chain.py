...
class ActionChain(object):
    ...
    def execute_double_loop(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        if phase.get_phase() == 7:
            ...
            if blue_area < BLUE_AREA_MIN_THRESHOLD:
                ...
                target_x = self.get_target_x_by_course(image, OFFSET_Y, self.opposite_course)
                left_speed, right_speed = self.calc_motor_speed(target_x, base_speed=BASE_SPEED)
                return (left_speed, right_speed), Mode.DOUBLE_LOOP
            elif dist_start >= FOURTH_INTERSECTION_LIMIT:
    ...
    def carry_bottle1_relative(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        # phase1: 赤ピクセル数 < 500 かつ距離 > 0 かつ距離 <= 10 で phase2へ。yaw基準セット
        if phase.get_phase() == 1:
        ...
        if phase.get_phase() == 6:
            ...
            if position_diff < threshold:
                ...
                if self.course_type == "upper" or position_diff < 1300:
                    ...
                    return (BASE_SPEED + 5, BASE_SPEED), Mode.CARRY_BOTTLE1
                else:
                    return (BASE_SPEED, BASE_SPEED + 5), Mode.CARRY_BOTTLE1
            # 一定値超えたら次フェーズへ
            print(f"[DEBUG] mode={Mode.CARRY_BOTTLE1.value} | phase={phase.get_phase()} | position_diff={position_diff} >= {threshold}")
            phase.next_phase(current_pos)
            return (0, 0), Mode.CARRY_BOTTLE1

        # phase7: 青ターゲット検出または最大回転量到達で次フェーズ。最低回転量以上の旋回処理。
        if phase.get_phase() == 7:
            position_diff = phase.get_position_diff(current_pos)
        ...
        if phase.get_phase() == 8:
            ...
            if blue_center is not None and blue_center[1] > 10:
                ...
                if in_tolerance:
                    print(f"[DEBUG] mode={Mode.CARRY_BOTTLE1.value} | phase={phase.get_phase()} | no_blue_target | yaw_ok")
                    phase.next_phase(current_pos)
                    return (0, 0), Mode.CARRY_BOTTLE1
                else:
                    if yaw_error < 0:
        ...
        # phase9: 青ターゲットのy座標が300になるまでゆっくり直進
        if phase.get_phase() == 9:
            # 標準的な距離計算を使用
            ...
            if blue_center is not None and blue_center[1] > 10:
                ...
                # 旧ロジック（青ターゲット未検出時に次フェーズへ遷移）をコメントアウト
        ...
        if phase.get_phase() == 12:
            ...
            distance_from_start = phase.get_position_diff(current_pos)
            # 計算距離到達で停止
            if distance_from_start >= self._calculated_distance + 20:
    ...
    def carry_bottle2_relative(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        if phase.get_phase() == 10:
            ...
            if blue_center is not None and blue_center[1] > 10:
                ...
                if in_tolerance:
                    # ヨー角OK→フェーズ11へ
                    # フェーズ11開始位置を設定
                    print(f"[DEBUG] mode={Mode.CARRY_BOTTLE2.value} | phase={phase.get_phase()} | no_blue_target | yaw_ok")
                    phase.next_phase(current_pos)
                    return (0, 0), Mode.CARRY_BOTTLE2
                else:
                    # ヨー角調整
        ...
        # phase13: もう一度青ターゲットを中心に合わせる（EYE_BLUEのフェーズ2と同じ処理）
        if phase.get_phase() == 13:
            blue_center, _, blue_pixel_count = find_blue_target_center(image)
        ...
        # phase14: 青ターゲット追跡または計算距離まで直進。距離到達または青検出で次フェーズへ。
    ...
    def eye_blue(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        if phase.get_phase() == 0:
            ...
            if blue_center is not None and blue_center[1] > 10:
                ...
                if abs(diff) <= 30:
                    et.set_start_yaw()
                    print(f"[DEBUG] mode={Mode.EYE_BLUE.value} | phase={phase.get_phase()} | centered | blue_center=({blue_center[0]}, {blue_center[1]}) | pixels={blue_pixel_count} | _calculated_distance={self._calculated_distance}")
                    phase.next_phase(current_pos)
        ...
        if phase.get_phase() == 2:
            ...
            if blue_center is not None and blue_center[1] > 10:
                ...
                in_tolerance, yaw_error = et.is_start_yaw_error_within(2.0)
                if in_tolerance:
                    print(f"[DEBUG] mode={Mode.EYE_BLUE.value} | phase={phase.get_phase()} | no_blue_target | yaw_ok")
                    phase.next_phase(current_pos)
                    return (0, 0), Mode.EYE_BLUE
                else:
                    if yaw_error < 0:
