...
class ActionChain(object):
    ...
    def carry_bottle1_relative(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        if not self._init:
            ...
            self.et.set_start_yaw_nearest_vertical_pole()  # 垂直のスタートヨーを設定
        et = self.et
        phase = self._phase

        # phase0: 赤ピクセル数 > 3000 で phase1へ。
        if phase.get_phase() == 0:
    ...
    def carry_bottle2_relative(self, image: np.ndarray) -> Tuple[SpeedTuple, Mode]:
        ...
        # phase4: ジャイロ補正。yaw誤差4.0以内で phase5へ。誤差大きい場合は単純に速度5で調整（eye_blueと同様、時間調整なし）。
        if phase.get_phase() == 4:
            in_tolerance, yaw_error = et.is_start_yaw_error_within(4.0)
            start_yaw = et.get_start_yaw()
            current_yaw = et.get_yaw()
        ...
        # 5. 直進（コース側モーターが所定値移動まで、両輪BASE_SPEED。所定値超えたらphase6へ、モーター位置記録）
        if phase.get_phase() == 5:
            current_pos = self.get_motor_position(self.course)
        ...
        # 6. 左旋回（コース側モーターが所定値移動まで、courseに応じて旋回方向決定。所定値超えたらphase7へ、モーター位置記録）
        if phase.get_phase() == 6:
            stop_turn = et.is_yaw_turn_finished(side=self.opposite_course, threshold_deg=90.0)
            if stop_turn:
        ...
        # 7. 直進（コース側モーターが所定値移動まで、両輪BASE_SPEED。所定値超えたらphase8へ、モーター位置記録、pre_target_x初期化）
        if phase.get_phase() == 7:
            current_pos = self.get_motor_position(self.course)
            position_diff = phase.get_position_diff(current_pos)
        ...
        # 8. 仮想ライン直進（コース側モーターが所定値移動まで仮想ライン中心座標取得処理、pre_target_x更新。所定値超えたらphase9へ、モーター位置記録）
        if phase.get_phase() == 8:
            current_pos = self.get_motor_position(self.course)
            position_diff = phase.get_position_diff(current_pos)
        ...
        # 9. 直進（コース側モーターが所定値移動まで、両輪BASE_SPEED。所定値超えたらphase10へ、モーター位置記録）
        if phase.get_phase() == 9:
            current_pos = self.get_motor_position(self.course)
            position_diff = phase.get_position_diff(current_pos)
        ...
        # 10. 左旋回（青ターゲット検出まで、最低回転量・最大回転量。条件満たせばphase11へ、右モーター位置記録）
        if phase.get_phase() == 10:
