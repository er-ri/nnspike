    ...
    def hight_speed_avoid(self, image: np.ndarray) -> Tuple[Optional[float], Optional[Tuple[int, int]], Mode]:
        ...
        if phase.get_phase() == 0:
            ...
            if yellow_pixel_count > 18000 and yellow_cx is not None:
                ...
                # 型安全化: yellow_cx[0]がint/float、またはtuple/listの入れ子でもint/floatのみ
                if isinstance(yellow_cx, (tuple, list)) and len(yellow_cx) > 0:
                    x_val = yellow_cx[0]
                    if isinstance(x_val, (int, float)):
                        target_x = x_val
                    elif isinstance(x_val, (tuple, list)) and len(x_val) > 0 and isinstance(x_val[0], (int, float)):
                        target_x = x_val[0]
                    else:
                        target_x = 0
                    # 型安全化: int/float以外は0に強制
